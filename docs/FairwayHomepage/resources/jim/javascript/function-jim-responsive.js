  ...
  .bind("renderresponsive", function(event, $element) {
	...
	$components.each(function(){
		...
		if (aspectRatioAttr || originalHeightAttr || dataYAttr || dataXAttr || dataSizeHeightAttr || dataSizeWidthAttr)
			item.data(newData);
		if (hasCSS && (!jimUtil.isRelative(item) || item.is(".masterinstance")))
			item.css(newCSS);
	...
	"resetOriginalTableSize" : function($table, resetWidth, resetHeight, cellSizeMap, newTableWidth, newTableHeight){
  	...
  	"refreshResponsiveComponents": function($component, onlySynced, sizeChange, updatePercentage, canvasLoad) {
		...
		var imagePercentageSelector = ".image.lockV.percentage, .image.lockH.percentage";
  		...
  		if(canvasLoad)
  			$newTarget = $newTarget.find(".masterInstance .percentage, .percentage.non-processed-percentage, " + imagePercentageSelector + ", " + relativePercentageSelector);
  		else {
			$newTarget = $newTarget.find(".masterinstance, .masterinstance .percentage, .percentage.shapewrapper, " + imagePercentageSelector + ", " + relativePercentageSelector);
		}
		...
		for(i=0, iLen=$newTarget.length; i<iLen; i+=1) {
			...
			var masterInstance = $t.closest(".masterinstance");
			if ($t.closest(".masterinstance").length > 0)
				masterInstances = masterInstances.add(masterInstance);
		}
		$newTarget = $newTarget.add(masterInstances);
		$newTarget = $newTarget.add($target.closest(".masterinstance"));
  		...
  		for(i=0, iLen=$newTarget.length; i<iLen; i+=1) {
  			...
  			switch($t.jimGetType()) {
				...
				case itemType.masterinstance:
					jimResponsive.refreshMasterInstance($t);
					break;
				case itemType.group:
	  ...
	  "getMasterInstancePosition" : function(item, w, h) {
		var masterL = parseFloat(item.css("left"));
		var masterT = parseFloat(item.css("top"));
		
		var hpin = jimPin.getHorizontalPin(item) ;
		var vpin = jimPin.getVerticalPin(item);
		if (hpin != "none" || vpin != "none") {
			var parentBounds = jimResponsive.getParentBounds(item, jimResponsive.getParentComponent(item));
			
			if (hpin == "center")
				masterL = masterL + (parentBounds.width / 2) - (w / 2);
			else if (hpin == "end")
				masterL = masterL + parentBounds.width - w;
			
			if (vpin == "center")
				masterT = masterT + (parentBounds.height / 2) - (h / 2);
			else if (vpin == "end")
				masterT = masterT + parentBounds.height - h;	

			var isFixed = item.css("position") == "fixed";
			if (isFixed) {
				masterL += $(".scrollable-root").scrollLeft();
				masterT += $(".scrollable-root").scrollTop();
			}
		}
		
		return {x: masterL, y: masterT};
	  },
	  "refreshMasterInstance" : function(item) {
		var $parent = jimResponsive.getParentComponent(item);
		var parentBounds = jimResponsive.getParentBounds(item, $parent);
		var newBounds = jimUtil.getRelativeItemBounds(item, parentBounds, true);
		var hpin = jimPin.getHorizontalPin(item) != "none";
		var vpin = jimPin.getVerticalPin(item) != "none";
		var masterPos = jimResponsive.getMasterInstancePosition(item, newBounds.width, newBounds.height);
		var isFixed = item.css("position") == "fixed";
		
		var offset = {x : - newBounds.x + masterPos.x, y: - newBounds.y + masterPos.y};
		
		jimUtil.moveRelativeItemChilds(item, offset);
		if (!hpin) {
			item.css("left", (newBounds.x - (isFixed ? $(".scrollable-root").scrollLeft() : 0)) + "px");
			if (!isFixed)
				item.data("offsetX", newBounds.x);
		}
		if (!vpin) {
			item.css("top", (newBounds.y - (isFixed ? $(".scrollable-root").scrollTop() : 0)) + "px");
			if (!isFixed)
				item.data("offsetY", newBounds.y);
		}
			
		item.css("width", newBounds.width + "px");
		item.css("height", newBounds.height + "px");
		
		if (item.find(".percentage").length > 0) {
			item.addClass("relativeWrapper");
			var responsiveSizeWrapper = item.find(".masterInstanceRelativeSizeWrapper");
			responsiveSizeWrapper.css("width", parentBounds.width + "px");
			responsiveSizeWrapper.css("height", parentBounds.height + "px");
		} else item.removeClass("relativeWrapper");
	  },
	  "refreshResponsiveRelativeItem" : function(item) {
		...
		relativeWrapper.css("width", (item.is(".hidden") ? 0 : newBounds.width) + "px");
		relativeWrapper.css("height", (item.is(".hidden") ? 0 : newBounds.height) + "px");		
