...
(function(window, undefined) {
  jQuery.extend(jimEvent.fn, {
    ...
    "jimGetFileValue": function($target) {
    	var self = this, i, iLen, $selected;
    	if(jimUtil.exists($target) && $target.length) {
    		switch($target.jimGetType()) {
    			case itemType.file:
    	        	if($target.find("input[type='file']").attr("readAsURL"))
    	        		return $target.find("input[type='file']").attr("readAsURL");
    	     }
    	}
    	return "";
    },
