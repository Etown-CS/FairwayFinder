	...
	/* START PINNING UTIL FUNCTIONS */
	  window.jimPin = {
		    ...
		    "pinElement" : function(elem, offsetX, offsetY, canvasLoad, pinChanges) {
				...
				var scrollYSize = 0;
		  		...
		  		if(!jsPin){
						...
						var newElem = elem.is(".dynamicpanel") ? elem.find(".panel.default") : elem;
						if (elem.data("widthUnit") == "%") {
							var width = elem.closest("#alignmentBox").innerWidth() * (elem.data("width")/100);
							scrollXSize = $("#simulation").innerWidth() - $("#simulation").children().first().innerWidth();
							elem.css("width", "calc(" + width + "px" + " - " + newElem.css("border-right-width") + ")");
						}
						
						if (elem.data("heightUnit") == "%") {
							var height = elem.closest("#alignmentBox").innerHeight() * (elem.data("height")/100);
							scrollYSize = $("#simulation").innerHeight() - $("#simulation").children().first().innerHeight();
							elem.css("height", "calc(" + height + "px" + " - " + newElem.css("border-top-width") + " - " + newElem.css("border-bottom-width") + ")");
							if (elem.css("min-height") != "0px")
								elem.css("min-height", "calc(" + elem.data("height") + "% - " + scrollYSize + "px - " + newElem.css("border-top-width") + " - " + newElem.css("border-bottom-width") + ")");
						}
						...
						var vPin = jimPin.getVerticalPin(elem);
				  		if((scrollDir == "vertical" && vPin=="none") || (scrollDir == "horizontal" && hPin=="none") || webOuterScroll){
					  		...
					  		if(oldPosition=="fixed"){
									...
									var newElem = elem.is(".dynamicpanel") ? elem.find(".panel.default") : elem;
									if (elem.data("widthUnit") == "%")
										elem.css("width", "calc(" + elem.data("width") + "% - " + newElem.css("border-left-width") + " - " + newElem.css("border-right-width") + ")");
									if (elem.data("heightUnit") == "%") {
										elem.css("height", "calc(" + elem.data("height") + "% - " + newElem.css("border-top-width") + " - " + newElem.css("border-bottom-width") + ")")
										if (elem.css("min-height") != "0px")
											elem.css("min-height", "calc(" + elem.data("height") + "% - " + newElem.css("border-top-width") + " - " + newElem.css("border-bottom-width") + ")");
									}
			...
			"calculateAbsoluteTop": function(target, margin, pinType, position, error) {
				...
				//bug in chrome?
				if(target.is("table") || target.is(".masterinstance"))
