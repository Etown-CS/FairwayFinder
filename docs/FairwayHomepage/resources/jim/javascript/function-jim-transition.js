...
(function (window, undefined) {
  ...
  function createHandler(sequential) {
    ...
    return function(transition, reverse, $to, $from) {
		...
		preLoading = function(){
			...
			var panel = jQuery($panels[t]);
      		if(panel.data('widthUnit') === "%") {
      			...
      			if(width!==undefined) {
      				panel.parent().css("width", "calc(" + width + "% - " + panel.css("border-left-width") + " - " + panel.css("border-right-width") + ")");
      				panel.css("width", "100%")
      			}
      		}
      		if(panel.data('heightUnit') === "%") {
      			...
      			if(height!==undefined) {
      				panel.parent().css("height", "calc(" + height + "% - " + panel.css("border-top-width") + " - " + panel.css("border-bottom-width") + ")");
      ...
      //load target first
		  var linkScreen, linkTemplate, linkMaster, linkScenario;
		  var waitCss=true;
		if ($to.hasClass("ui-scenario")) {
		  $to.removeClass("visible").addClass("invisible");
          $to.addClass(jimMain.defaults.activePageClass);
		  if($.browser.msie && $.browser.version<=8){
			linkScenario = jQuery($to).find("link:first").next()[0];
          } else {
			linkScenario = jQuery($to).find("link:first")[0];
          }
		  $to.css("display","none");
        } else if($to.hasClass("ui-page")) {
          $to.removeClass("visible").addClass("invisible");
          $to.addClass(jimMain.defaults.activePageClass);
          if($.browser.msie && $.browser.version<=8){
        ...
        /*check if css of screen and template are loaded*/
		  (function(){
				...
				if(waitCss && linkScenario){
				  if(!checkLoadMark(linkScenario,"scenario")){
				    setTimeout(arguments.callee, 20);
	                return;
				  }
				}
  ...
  function checkLoadMark(link,type){
	  ...
	  if($.browser.msie && $.browser.version<=8){
	      ...
	      if(type=="screen")
	           ...
	           return jQuery(".ui-scenario:last > #loadMark").css('width') == '1px';
	      else if(type="master")
