...
(function (window, undefined) {
  ...
  function checkExternalClick(event, data) {
	...
	if($target.closest(".jim-web-dd").length == 0) {
		...
		unbindClickEvents();
		var value = $dropdown.find(".valign").children(".value").text();
  ...
  /*********************** START DROPDOWN METHODS ************************/
	...
	function bindDropDown(element) {
		jQuery(".jim-web-dd").on("refreshDropDown", function(event, target) {
			if ($(target).length > 0) {
				var $dd = $(target);
				fillDropDownOptions($dd);
			}
		});
	...
	function fillDropDownOptions($target) {
		...
		switch(type) {
			case itemType.dropdown:
	            ...
	            jQuery(".jim-web-dd .dd_content").css("letter-spacing", $target.css('letter-spacing'));
	            jQuery(".jim-web-dd .dd_content").css("text-transform", $target.css('text-transform'));
