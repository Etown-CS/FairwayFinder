(function (window, undefined) {
	...
	function bindTimePeriods(tp) {
		// Fix for mouse up in top and bottom areas, outside main .ti_periods
		tp.parent().on("mouseup", function(event, data) {
		...
		tp.find(".ti_periods").on('drag', function(event, data) {
			if(dragStart) {				
				var offset = jQuery(event.target).parent().parent().offset(),
				dd_height = parseInt(jQuery(event.target).parent().parent().parent().css('height')),
				zoom = jimDevice.getZoom(),
				posX = event.pageX,
				posY = event.pageY;
